...
@Component({
  selector: 'app-career-pages-link',
  templateUrl: './career-pages-link.component.html',
  styleUrls: ['./career-pages-link.component.css'],
})
export class CareerPagesLinkComponent implements OnInit {
  ...
  getAllStudyMaterialLinks(allSavedLinks: Tab[]): void {
    this.careerPageList = allSavedLinks.filter(
      (link) => link.categoryId === LinkCategory.Career_Page
    );
  }

  todoAction(actionIndex: number, linkIndex: number): void {
    const index = this.savedTabs.findIndex(
      (saveTab) => saveTab == this.careerPageList[linkIndex]
    );
  ...
  setStatus(statusIndex: number, tabIndex: number): void {
    const index = this.savedTabs.findIndex(
      (saveTab) => saveTab == this.careerPageList[tabIndex]
    );
